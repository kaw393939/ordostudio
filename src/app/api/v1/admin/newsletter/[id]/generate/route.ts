...
import { generateNewsletterSchema } from "../../../../../../../lib/api/schemas";
import { generateNewsletterDraft, NewsletterIssueNotFoundError } from "../../../../../../../lib/api/newsletter";
...
async function _POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = parsePayload(generateNewsletterSchema, raw, request);
  if (!parsed.success) return parsed.response;

  try {
    const updated = generateNewsletterDraft({
      id,
      actor: asUserActor(auth.user.id),
      requestId: crypto.randomUUID(),
      fieldReportIds: parsed.data.field_report_ids,
      researchUrls: parsed.data.research_sources,
    });
    ...
    return problem(
      {
        ...
        status: 500,
        detail: "Unable to generate newsletter draft.",
      },
      request,
    );
  }
